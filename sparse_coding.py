...
def load_word_embeddings(path='glove.6B.50d.txt'):
    ...
    df.drop(df.index[14798], inplace=True) # Stupid word is called "nan", causes bugs
    return df
...
def learn_dictionary(X):
    ...
    #                                 transform_n_nonzero_coefs=5, verbose=2, n_jobs=num_cpus)
    dictionary = MiniBatchDictionaryLearning(n_components=2000, fit_algorithm='lars', transform_algorithm='lars',
                                             transform_n_nonzero_coefs=5, verbose=2, n_jobs=num_cpus, batch_size=16, n_iter=1000)
...
def analyze_atoms_of_discourse(model_file, embeddings_file, words=["bank", "blue", "credit", "fly", "light", "mean", "star", "tie", "abrogate", "ceramics", "connecticut", "imperfect", "sleepless", "spoken"]):
...
# Uncomment for analyzing the learned model

if __name__ == "__main__":
    #analyze_atoms_of_discourse("results/glove_10000_n500_k3/dictionary_10000_n500_k3.p", "results/glove_10000_n500_k3/selected_df_10k.p", words=["tie", "star", "bank", "cut", "bass", "connecticut"])
    #analyze_atoms_of_discourse("results/glove_100000_n500_k3_lars/dictionary_100000_n500_k3.p", "results/glove_100000_n500_k3_lars/selected_df_100k_k3.p", words=["tie", "star", "bank", "cut", "bass", "connecticut"])
    #analyze_atoms_of_discourse("results/glove_100000_n500_k3/dictionary_100000_n500_k3.p", "results/glove_100000_n500_k3/selected_df_100k_k3.p")
    analyze_atoms_of_discourse("results/glove_100000_n2000_k5_lars/dictionary_100000_n2000_k5.p", "results/glove_100000_n2000_k5/selected_df_100k_k5.p", words=["bank", "blue", "credit", "fly", "light", "mean", "star", "tie", "abrogate", "ceramics", "connecticut", "imperfect", "sleepless", "spoken", "cut", "bass"])



# Uncomment for learning the Sparse Coding Model

# if __name__ == "__main__":
#     df = load_word_embeddings()
#     filtered = get_only_alpha(df)
#     print(filtered.shape)
#     selected_df = sample_from_dataframe(filtered, include_list=["bank", "blue", "credit", "fly", "light", "mean", "star", "tie", "abrogate", "ceramics", "connecticut", "imperfect", "sleepless", "spoken", "cut", "bass"], n=100000)
#     print(selected_df.shape)
#     # Save the dataframe for Ruochi:
#     selected_df.to_pickle("selected_df_100k_k5.p")
#     selected_df.to_csv("selected_df_100k_k5.csv", header=False)

#     X = selected_df.as_matrix()
#     dictionary = learn_dictionary(X)
#     save_dict(dictionary, "dictionary_100000_n2000_k5.p")

#     dictionary = load_dict("dictionary_100000_n2000_k5.p")
...
#     # 100000 words, 2000 components, k=5, Dictionary learning took  0 hours 25 mins 44 secs, 1000 iterations
